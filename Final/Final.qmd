...
Profile['w_prime_rho_v_prime_bar'] = Profile['$LE$']/Profile['Lv']
...
```{python}
Profile['F_CO2_g'] = Profile['Fc']*44.01*1e-3*1800
Profile['F_CO2_g']
```
...
Profile['Beta'] = Profile['$H$']/Profile['$LE$']
Profile['Beta'] 
...
* Reynolds decomposition partitions a time series into two components, the mean value and the instantaneous deviation from the mean at any given point in time.  This allows us to investigate turbulent fluctuations of atmospheric phenomena.  It also us to assess turbulent transport of scalars, by calculating the covariance between two properties, such as vertical wind speeds and carbon dioxide concentrations.
...
for p,m in zip(['a','b','c','d'],[1,2,3,4]):
  df = pd.DataFrame(data={'u':u*m,'w':w*m})

  df['u_prime'] = df['u']-df['u'].mean()
  df['w_prime'] = df['w']-df['w'].mean()
  u_prime_w_prime_bar = (df['u_prime']*df[f'w_prime']).mean()
  u_star = (-u_prime_w_prime_bar)**.5

  print(p)
  print(df.round(4))
  print('u_prime_w_prime_bar',np.round(u_prime_w_prime_bar,4),'\nu_star',np.round(u_star,4))
